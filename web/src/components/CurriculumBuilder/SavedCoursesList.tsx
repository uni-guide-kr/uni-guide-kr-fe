...
import { Plus, BookOpen, ChevronUp, ChevronDown, GripHorizontal, Star, BookMarked } from 'lucide-react';
...
export const SavedCoursesList: React.FC<SavedCoursesListProps> = ({
  ...
  const getCategoryLabel = (category: string) => {
    switch (category) {
      ...
      case 'general-required': return '교양필수';
      case 'general-elective': return '교양선택';
                ...
                <div className="flex gap-2">
                  ...
                  <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="general-required">교양필수</SelectItem>
                      <SelectItem value="general-elective">교양선택</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {courses.length === 0 ? (
          ...
          <div className="space-y-3">
            {courses.map((course) => {
              const prerequisites = getCoursesByIds(course.prerequisites);
              const recommended = getCoursesByIds(course.recommendedCourses);
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm mb-3">
                        <div>
                          <span className="text-gray-500">강의평 평점</span>
                      ...
                      {(prerequisites.length > 0 || recommended.length > 0) && (
                        <div className="border-t pt-3 space-y-2">
                          {prerequisites.length > 0 && (
                            <div>
                              <span className="text-xs text-gray-500">선수과목:</span>
                              <div className="flex flex-wrap gap-1 mt-1">
                                {prerequisites.map((prereq) => (
                                  <Badge key={prereq.id} variant="outline" className="text-xs bg-red-50">
                                    {prereq.name}
                                  </Badge>
                                ))}
                              </div>
                          ...
                          {recommended.length > 0 && (
                            <div>
                              <span className="text-xs text-gray-500">추천과목:</span>
                              <div className="flex flex-wrap gap-1 mt-1">
                                {recommended.map((rec) => (
                                  <Badge key={rec.id} variant="outline" className="text-xs bg-blue-50">
                                    {rec.name}
                                  </Badge>
                                ))}
                              </div>
