...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  }, [userProfile, requirementRule, semesterPlans]);
  ...
  const calculateProgress = () => {
    ...
    const allPlannedCourseIds = semesterPlans.flatMap((p) => p.courses);
    ...
    const areaProgress = requirementRule.areas.map((area) => {
      ...
      if (area.courses) {
        ...
        const completedInArea = completedCourseIds.filter((id) => area.courses!.includes(id));
        ...
        const completedCourses = getCoursesByIds(completedCourseIds).filter(
    ...
    planCourses.forEach((course) => {
      ...
      if (unmetPrereqs.length > 0) {
        ...
        issues.push({
          ...
          severity: 'error',
          courseId: course.id,
          message: `${course.name}의 선수과목이 충족되지 않았습니다: ${prereqNames}`,
  ...
  const initializeCurriculumPlan = () => {
    ...
    // Add default courses to first two semesters (1-1, 1-2)
    // 1학년 1학기: 프로그래밍기초, 대학영어1, 글쓰기
    semesters[0].courses = ['cs101', 'gen101', 'gen103'];
    // 1학년 2학기: 자료구조, 대학영어2, 현대사회와윤리, 경제학개론
    semesters[1].courses = ['cs201', 'gen102', 'gen201', 'gen202'];
